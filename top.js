...
class SearchA {
    ...
    getNeighbors(parent, target) {
     ...
     run() {
        ...
        this.moveBottomTowLine(solutionBottomTwo)
    };
     moveBottomTowLine(solutionBottomTwo) {
     ...
     lastOneMove(originZero) {
        ...
        for (let i = 0; i < this.bottomlinePath.length; i++) {
            ...
            nextStep = [element[0] + originZero[0], element[1] + originZero[1]];
             ...
             movePiece(nextStep[0], nextStep[1]);
     ...
     actionMove(targetPath) {
        for (let i = 0; i < targetPath.length - 1; i++) {
            this.ZeroPathMove(targetPath[i + 1]);
            const targetPosition = targetPath[i];
            const zeroPosition = targetPath[i + 1];
             movePiece(targetPosition[0], targetPosition[1]);
            [
     ...
     ZeroPathMove(targetPath) {
        ...
        for (let index = 0; index < zeroPath.length - 1; index++) {
            ...
            const numPosition = zeroPath[index + 1];
             movePiece(numPosition[0], numPosition[1]);
